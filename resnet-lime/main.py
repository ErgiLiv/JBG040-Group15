...
def explain_with_lime(model, device, num_samples=1000, num_features=10):
    """
    ...
    # Define transformations (ResNet input size: 128x128)
    ...
    def predict_fn(images):
        """Convert batch of images to model predictions."""
        images = torch.tensor(images).permute(0, 3, 1, 2).float()  # Convert to PyTorch tensor
        outputs = model(images) 
        return torch.nn.functional.softmax(outputs, dim=1).detach().numpy()  # Convert to probabilities

    # Initialize LIME explainer
    explainer = lime_image.LimeImageExplainer()

    # Explain prediction for the test image
...
def load_model(weights_path: str):
    ...
    model.load_state_dict(torch.load(weights_path, map_location="cpu", weights_only=True)) # Load trained weights
    model.eval()
    return model
...
def main(args: argparse.Namespace, activeloop: bool = True) -> None:
    ...
    # visualize_lime(model, device, test_sampler)
...
if __name__ == "__main__":
    ...
    model = load_model("model_weights/resnet18_model_03_13_1_38.pt")  # Load model properly
    explain_with_lime(model, "example_image.jpg")  # Call LIME function
