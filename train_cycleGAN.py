...
TRAINING_EPOCHS = 100  # Reduce for CPU training
...
if not os.path.exists(f"checkpoints/{MODEL_NAME}"):
    ...
    run_command([
        ...
        "--direction", "AtoB",  # ✅ Ensure correct training direction
        "--dataset_mode", "unaligned",  # ✅ Train on unaligned dataset
        "--load_size", "128",  # ✅ Resize images to match input shape
        "--crop_size", "128"
    ])
else:
    print("✅ CycleGAN already trained. Skipping training.")

# Ensure checkpoints exist before running the test script
if not os.path.exists(f"checkpoints/{MODEL_NAME}/latest_net_G.pth"):
    raise FileNotFoundError(f"❌ Error: Checkpoints not found for {MODEL_NAME}. Training may have failed.")
